...
for col in X.columns:
    missing_indices = X[col].isnull()  # Find missing values indices
    filled_values = X[~missing_indices][col].values  # Get non-missing values
    X.loc[missing_indices, col] = np.random.choice(filled_values, size=np.sum(missing_indices), replace=True)
...
plt.savefig(os.path.join(folder_path, 'LIMS:1110SUEZ0002.SALT - Hotdeck.png'))
plt.close()
